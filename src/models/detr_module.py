...
class DETRModule(LightningModule):
    ...
    def __init__(
        ...
        self.val_coco = COCO(val_coco.path)
        self.coco_evaluator = CocoEvaluator(self.val_coco, ["bbox"])
    ...
    def training_step(self, batch: Any, batch_idx: int):
        logits, y = self.model_step(batch)
    ...
    def validation_step(self, batch: Any, batch_idx: int):
        logits, target = self.model_step(batch)
        loss = self.criterion(logits, target)

        target_sizes = torch.stack([T["orig_size"] for T in target])
        pred = self.postprocessors["bbox"](logits, target_sizes)
        res = {t["image_id"].item(): output for t, output in zip(target, pred)}
    ...
    def on_validation_epoch_end(self):
        ...
        log_dict = {f"val/{k}": m.global_avg for k, m in self.val_metric.meters.items()}
        log_dict.update(
            {
                i: j
                for i, j in zip(
                    ...
                    self.coco_evaluator.coco_eval["bbox"].stats,
                )
            }
        )
